...
The DAG creation layer is orchestrated by [`apps/udp/composer/dags/udp_dag_factory.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/composer/dags/udp_dag_factory.py).
...
2. **Dual-Path Configuration Discovery**:
   ...
   * **Path 2 (Fallback - Local Filesystem)**: Scans local directories (`apps/udp/configs/sources/` or `/home/airflow/gcs/dags/configs/sources/`).
...
1. **GCS Prefix Dynamic Binding** — [`apps/udp/composer/dags/udp_dag_factory.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/composer/dags/udp_dag_factory.py):
   ```python
   {"name": "GCS_PREFIX", "value": f"{cfg['source_system']}/raw/{target_table}/dt={{{{ ds }}}}"}
   ```
2. **Timestamped Parquet Key** — [`apps/udp/cloud-run-jobs/common/base_extractor.py:L161`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/base_extractor.py#L161):
   ```python
   filename = f"{prefix.strip('/')}/data_{now_utc.strftime('%Y%m%d_%H%M%S')}.parquet"
   ```
3. **Airflow Partition Ingestion Pattern** — [`apps/udp/composer/dags/udp_dag_factory.py:L156`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/composer/dags/udp_dag_factory.py#L156):
   ```python
   source_objects=[f"{cfg['source_system']}/raw/{cfg['target_table']}/dt={{{{ ds }}}}/*.parquet"]
   ```
4. **Airflow XCom Job Manifest Key** — [`apps/udp/cloud-run-jobs/common/job_runner.py:L70`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/job_runner.py#L70):
   ```python
...
[`apps/udp/cloud-run-jobs/common/`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/)

### Core Modules Breakdown

1. **[`base_extractor.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/base_extractor.py) (`BaseExtractor`)**:
...
2. **[`job_runner.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/job_runner.py) (`JobRunner`)**:
...
3. **[`job_config.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/cloud-run-jobs/common/job_config.py) (`IngestionJobConfig`)**:
...
Each job Dockerfile is built using `apps/udp/cloud-run-jobs` as the build context root. This allows Docker to copy the shared `common/` module alongside the job's code:
...
Located under `apps/udp/configs/sources/<system>/<table_name>.yaml`:
...
Located under `apps/udp/configs/schema/<system>/<table_name>_schema.json`:
...
[`apps/udp/utilities/test_udp_end_to_end.py`](file:///usr/local/google/home/anikethd/elementl/ELEMENTL/elementl-data-ingestion-fw/apps/udp/utilities/test_udp_end_to_end.py)

```bash
python3 apps/udp/utilities/test_udp_end_to_end.py
```
...
gcloud builds submit   --config=apps/udp/cloud-run-jobs/suiteQL-ingestion/cloud_build.yaml   apps/udp/cloud-run-jobs
```

### Onboarding a New Source Table (Step-by-Step)

1. **Step 1 — Create Schema Definition**:
   Add `apps/udp/configs/schema/<system>/<table_name>_schema.json` with field names and data types.
2. **Step 2 — Create Declarative YAML**:
   Add `apps/udp/configs/sources/<system>/<table_name>.yaml`:
   ```yaml
...
3. **Step 3 — Upload to GCS**:
   ```bash
   gcloud storage cp apps/udp/configs/sources/<system>/<table_name>.yaml gs://bkt-*-udp-configs/sources/<system>/
   ```
