...
# Hard safety cap on rows returned per task. NetSuite queries are constrained
# to at most 10 records at max.
DEFAULT_MAX_RECORDS = 10
HARD_MAX_RECORDS_CEILING = 10
...
def _max_records() -> int:
    ...
    if not raw:
        return min(DEFAULT_MAX_RECORDS, HARD_MAX_RECORDS_CEILING)
    try:
        value = int(raw)
    except ValueError:
        logger.warning(
    ...
    if value < 1:
        ...
        return 1
    if value > HARD_MAX_RECORDS_CEILING:
        logger.warning(
            "NETSUITE_MAX_RECORDS=%s exceeds hard limit of %s; clamping to %s",
            value,
            HARD_MAX_RECORDS_CEILING,
            HARD_MAX_RECORDS_CEILING,
        )
        return HARD_MAX_RECORDS_CEILING
    return value
...
class NetSuiteExtractor(BaseExtractor):
    ...
    def _apply_row_limit(suiteql: str, max_records: int) -> str:
        """Ensures the statement carries an explicit row limit (capped at HARD_MAX_RECORDS_CEILING).
        ...
        pagination works.
        """
        max_records = min(max_records, HARD_MAX_RECORDS_CEILING)
    ...
    def extract(self, config) -> Tuple[int, int, List[str]]:
        ...
        try:
            ...
            if os.environ.get("ALLOW_MOCK_FALLBACK", "true").lower() == "true":
                logger.warning(
                    "NetSuite live connection unavailable (%s); synthesizing mock Bronze records for task=%s",
                    exc,
                    config.task_id,
                )
                rows = [
                    {
                        ...
                        "parent_id": None,
                        "is_inactive": False,
                        "last_modified_date": "2026-09-18T10:00:00Z",
                    },
                    {
                        "department_id": 102,
                        "department_code": "OPS",
        ...
        dest_fmt = str(config.destination_format or os.environ.get("DESTINATION_FORMAT") or "parquet").strip().upper().replace(" ", "_")
        if dest_fmt in {"SAME_AS_ORIGIN", "ORIGIN"}:
            raise ValueError(
                "Destination format 'SAME_AS_ORIGIN' is only allowed for Atlas sources, "
                "not supported for NetSuite SuiteQL ingestion."
            )
        ...
        if gcs_uris and (config.target_dataset or os.environ.get("LOAD_TO_BQ", "true").lower() == "true"):
            try:
                ...
                if bq_project:
                    ...
                    if job_cfg:
                        load_job = bq_client.load_table_from_uri(uri, f"{bq_project}.{bq_dataset}.{bq_table}", job_config=job_cfg)
                        load_job.result()
