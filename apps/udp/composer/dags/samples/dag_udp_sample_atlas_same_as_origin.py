...
"""Sample Ingestion DAG for Atlas Native SAME_AS_ORIGIN format (Tested on Composer v3)."""
...
with DAG(
    dag_id="dag_udp_sample_atlas_same_as_origin",
    default_args=default_args,
    description="Sample configuration demonstrating SAME_AS_ORIGIN format preserving native GeoJSON envelopes (Atlas only).",
    schedule=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["udp", "atlas", "sample_atlas_same_as_origin", "SAME_AS_ORIGIN", "composer_v3"],
) as dag:
    ...
    with TaskGroup(group_id="ingestion_group", tooltip="Cloud Run Ingestion + Bronze BQ Load") as ingestion_group:
        execute_arcgis_ingestion = CloudRunExecuteJobOperator(
            ...
            overrides={
                "container_overrides": [
                    {
                        "env": [
                            {"name": "TASK_ID", "value": "sample_atlas_same_as_origin"},
                            ...
                            {"name": "TARGET_TABLE", "value": "sample_atlas_same_as_origin"},
                            {"name": "WHERE_CLAUSE", "value": "1=1"},
                            {"name": "MAX_RECORDS", "value": "10"},
                            {"name": "GCS_BUCKET", "value": RAW_BUCKET},
                            {"name": "GCS_PREFIX", "value": "atlas/raw/sample_atlas_same_as_origin/dt={{ ds }}"},
                            {"name": "EXECUTION_DATE", "value": "{{ ds }}"},
                            {"name": "RUN_ID", "value": "{{ run_id }}"},
                            {"name": "DAG_ID", "value": "{{ dag.dag_id }}"},
                            {"name": "DESTINATION_FORMAT", "value": "SAME_AS_ORIGIN"},
                            ...
                            {"name": "ENDPOINT", "value": "https://earthquake.usgs.gov/arcgis/rest/services/haz/Qfaults/MapServer"},
                            {"name": "LAYER_ID", "value": "21"},
                        ]
                    }
                ]
            },
        ...
        load_gcs_to_bq_bronze = GCSToBigQueryOperator(
            ...
            bucket=RAW_BUCKET,
            source_objects=["atlas/raw/sample_atlas_same_as_origin/dt={{ ds }}/*.parquet"],
            destination_project_dataset_table=f"{PROJECT_ID}.ds_bronze_atlas.sample_atlas_same_as_origin",
