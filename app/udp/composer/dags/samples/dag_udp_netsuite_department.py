...
"""Sample Ingestion DAG for NetSuite Department (Tested on Composer v3)."""
...
with DAG(
    dag_id="dag_udp_netsuite_department",
    default_args=default_args,
    description="NetSuite Department Master Hierarchy (SuiteQL Ingestion)",
    schedule=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["udp", "netsuite", "netsuite_department", "composer_v3"],
) as dag:

    with TaskGroup(group_id="ingestion_group", tooltip="Cloud Run Ingestion + Bronze BQ Load") as ingestion_group:
        execute_suiteql_ingestion = CloudRunExecuteJobOperator(
            task_id="execute_suiteql_ingestion",
            ...
            overrides={
                "container_overrides": [
                    {
                        "env": [
                            {"name": "TASK_ID", "value": "netsuite_department"},
                            {"name": "SOURCE_TYPE", "value": "netsuite"},
                            {"name": "SOURCE_SYSTEM", "value": "netsuite"},
                            {"name": "SOURCE_TABLE", "value": "department"},
                            {"name": "TARGET_DATASET", "value": "ds_bronze_netsuite"},
                            {"name": "TARGET_TABLE", "value": "netsuite_department"},
                            {"name": "WHERE_CLAUSE", "value": "ROWNUM <= 10"},
                            {"name": "MAX_RECORDS", "value": "10"},
                            {"name": "GCS_BUCKET", "value": RAW_BUCKET},
                            {"name": "GCS_PREFIX", "value": "netsuite/raw/netsuite_department/dt={{ ds }}"},
                            ...
                            {"name": "QUERY", "value": "SELECT * FROM department WHERE ROWNUM <= 10"},
                            {"name": "CONNECTION_SECRET_ID", "value": "secret-netsuite-api-config"},
                        ]
                    }
                ]
            },
        ...
        load_gcs_to_bq_bronze = GCSToBigQueryOperator(
            ...
            bucket=RAW_BUCKET,
            source_objects=["netsuite/raw/netsuite_department/dt={{ ds }}/*.parquet"],
            destination_project_dataset_table=f"{PROJECT_ID}.ds_bronze_netsuite.netsuite_department",
        ...
        execute_suiteql_ingestion >> load_gcs_to_bq_bronze
