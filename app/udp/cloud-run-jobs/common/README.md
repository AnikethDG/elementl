...
When building container images for Cloud Run, the build context is set to `app/udp/cloud-run-jobs/` so that `common/` is bundled alongside the specific job code:

```bash
# Example: Building suiteQL-ingestion from app/udp/cloud-run-jobs
gcloud builds submit \
  --config=suiteQL-ingestion/cloud_build.yaml \
  app/udp/cloud-run-jobs
```
