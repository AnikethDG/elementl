...
| `infra/udp/bigquery_datasets.tf` | Provisions only the **BigQuery ingestion & operational datasets** (`ds_bronze_oracle_p6`, `ds_bronze_netsuite`, `ds_bronze_atlas`, `ds_operations`) and **ingestion tables**: the 15 Oracle P6 Bronze tables (`google_bigquery_table.bronze_oracle_p6_tables`), the 11 NetSuite Bronze tables (`google_bigquery_table.bronze_netsuite_tables`), `ds_operations.ingestion_execution_logs`, and `ds_operations.ingestion_watermarks`. (Silver/Gold/Dataform tables and datasets are managed separately by Dataform). |
